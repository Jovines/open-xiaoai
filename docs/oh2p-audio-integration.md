...
这次受控句也再次证明“有 reference”不等于“ASR 文本可靠”：后台首次处理时，目标句中的“原有”被 Qwen3-ASR-1.7B 识别成“仍有”，而三路 SenseVoice 候选全部正确。随后对照重跑中，Qwen 直接听几乎无削波的数字 reference 又识别成“也有”，重听麦克风回录则识别正确；因此短词语义替换不能全部归因于房间噪声或外放回声，Qwen 单次结果不能作为事实。麦克风测试片段另有约 1.87% 样本达到满幅，表明当前 `volume=256` 也存在过度放大，需要独立降增益验证。当前保留原音、多模型候选、`fallible_asr` 标记和 `needs_review` 的策略是必要的。因为尚未执行播放时真人插话，处理器把麦克风与播放重叠段保守标为 `unknown + aec_not_yet_applied`，没有误报为已确认的 `overlap`。
