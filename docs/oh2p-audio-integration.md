...
2026-08-27 改为“声学层融合、语言层只转一次”：三路 VAD 至少两路支持后，逐语音段做 GCC-PHAT 对齐；全句固定相干性最佳的参考通道并保留 70% 权重，其他两路合计 30%，坏通道自动降权。等权版本曾把正确的“小爱”改成“小雅”，因此未上线。参考保护版本在 4 条人工真值三麦样本上 Qwen micro-CER 为 0、关键短语命中率 100%、RTF 0.412；报告保存在 NAS `录音/evaluation/results/array-controlled-reference70.json`。样本规模不足以证明普遍最优，后续人工校对必须继续回归。生产不再运行三路 SenseVoice 转写或 FireRed 仲裁；阵列质量合格的普通文本可直接用于低风险理解，高影响事项仍由 `fallible_asr` 契约要求回听、上下文交叉验证或询问。因为尚未执行播放时真人插话，处理器仍把麦克风与播放重叠段保守标为 `unknown + aec_not_yet_applied`，没有误报为已确认的 `overlap`。
