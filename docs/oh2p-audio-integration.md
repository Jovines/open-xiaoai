...
这次受控句也再次证明“有 reference”不等于“ASR 文本可靠”：后台首次处理时，目标句中的“原有”被 Qwen3-ASR-1.7B 识别成“仍有”，而三路历史对照模型候选全部正确。随后对照重跑中，Qwen 直接听几乎无削波的数字 reference 又识别成“也有”，重听部分麦克风回录则识别正确；因此短词语义替换不能全部归因于房间噪声或外放回声，Qwen 单次结果不能作为事实。早期生产为此分别转写三支麦克风并要求四份文本逐字相同，结果 2026-08-27 当天 22 条语音全部被标为 `needs_review`，门槛对普通家庭理解明显过严。

2026-08-27 先上线过“声学层融合、语言层只转一次”：三路 VAD 至少两路支持后，逐语音段做 GCC-PHAT 对齐；全句固定相干性最佳的参考通道并保留 70% 权重，其他两路合计 30%。等权版本曾把正确的“小爱”改成“小雅”，参考保护版本在 4 条人工真值三麦样本上 Qwen micro-CER 为 0、关键短语命中率 100%。进一步把三个物理通道全部补测后，麦克风 0 为 CER 6.10%/关键短语 75%，麦克风 1、麦克风 2 和参考保护融合均为 CER 0/关键短语 100%；最佳方向随声源位置变化，小样本也没有证明融合胜过最佳单麦。

同日根据 Zeris 主脑负责认知的边界继续收敛：生产改为三个方向使用同一语音边界、分别由 Qwen 转写一次，三份文本与三份精确 ASR 输入都作为 `fallible_asr` 观察交给 Zeris。采集层不挑赢家、不要求一致、不因差异自动判整段有问题；提示词要求主脑直接结合上下文理解，只在差异实质改变结论时保留对应不确定性。GCC-PHAT 融合音频继续归档供阵列研究和回听，但不再覆盖方向观察。生产不运行 SenseVoice 或 FireRed 仲裁。因为尚未执行播放时真人插话，处理器仍把麦克风与播放重叠段保守标为 `unknown + aec_not_yet_applied`，没有误报为已确认的 `overlap`。

同句三档采集增益对照显示，语音片段满幅削波从 `volume=256` 的 1.581973% 降至 `volume=128` 的 0.062687%，最终 `volume=96` 为 0.002811%，约比旧值减少 560 倍；Qwen 和 SenseVoice 的房间回录 CER 均未退化。因此生产固定使用可配置的 `RECORDER_PCM_GAIN=96`，约保留 8.5 dB 头部空间，不使用会改写证据动态范围的自动压缩。systemd recorder 同时改为 `KillMode=mixed`，实测升级重启时 46 秒残段能正常封口，不再因 ffmpeg 同时收到 SIGTERM 而丢失当前块。
