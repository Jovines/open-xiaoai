...
转写后端可用 `QWEN_ASR_DEVICE=cpu|cuda` 切换。CPU 模式使用 FP32 且不占显存；CUDA 模式使用 BF16，并把三路方向麦克风组成一个推理批次，适合主脑已经切到云端、可临时释放本地 GPU 的阶段。两种模式生成相同结构的可审计证据，并在 `provenance.directional_asr.device` 记录实际设备。
